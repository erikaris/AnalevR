...
RUN apt-get update -y && apt-get install -y apt-transport-https && rm -r /var/lib/apt
RUN echo "deb https://cloud.r-project.org/bin/linux/ubuntu xenial/" | tee -a /etc/apt/sources.list
...
RUN Rscript -e 'install.packages(c("devtools", "rzmq", "jsonlite", "xtable", "base64enc", \
